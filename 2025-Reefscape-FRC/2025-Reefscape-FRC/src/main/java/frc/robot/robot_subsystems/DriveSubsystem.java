...
public class DriveSubsystem extends EntechSubsystem {
    ...
    {
        // load the RobotConfig from the GUI settings (maybe store in Constants file)
        try{
        ...
        AutoBuilder.configure(
            ...
            new PPHolonomicDriveController(
                new PIDConstants(.04, 0.0, 0.0), 
                new PIDConstants(1.0, 0.0, 0.0)
            ),
            config, // robot configuration
            () -> {
                // this will flip the path followed to the red side of the field
                // the origin will remain on the blue side of the field
            ...
            // Handle exception as need
            DriverStation.reportError("Failed to lead PathPlanner config and configure AutoBuilder", e.getStackTrace());
        }
