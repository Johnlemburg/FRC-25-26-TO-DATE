...
public class BaseSparkMaxPositionSubsystem {
    ...
    public void configure(SparkMax spark, PositionControllerConfig config) {
        configure(spark,
                config,
                spark.getReverseLimitSwitch(/*Type.kNormallyOpen*/), // TODO: DO I NEED LIMIT SWITCH TYPE???
                spark.getForwardLimitSwitch(/*Type.kNormallyOpen*/), // TODO: DO I NEED LIMIT SWITCH TYPE???
                spark.getEncoder());
    }

    public void configure(SparkMax spark, PositionControllerConfig config, SparkLimitSwitch lowerLimit,
    ...
    private void setP(double p) {
        spark.getClosedLoopController().setP(p);
    }

    private double getP() {
        return spark.getClosedLoopController().getP();
    }*/
    ...
    private void updateRequestedPosition() {
        if (requestedPosition.isPresent()) {
            ...
            spark.getClosedLoopController().setIAccum(0);
        }
    }
}
